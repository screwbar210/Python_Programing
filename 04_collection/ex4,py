...
# 2️ 가장 많이 공부한 시간 출력하기
print(max(hours))  # ✅ 6시간

# 3️ 가장 많이 공부한 요일 출력하기
print(days[hours.index(max(hours))])  # ✅ 토요일

_, day = max(zip(hours, days))
print(f"{day}요일")

# 4️ 가장 높은 점수와 가장 낮은 점수 출력하기
scores = (90, 85, 78, 92, 88, 76)

print(
    f"최대 점수: {max(scores)}점, 최소 점수: {min(scores)}점"
)  # ✅ max 점수: 92점, min 점수: 76점

sortedScores = sorted(scores)

print(
    f"최대 점수: {sortedScores[-1]}점, 최소 점수: {sortedScores[0]}점"
)  # ✅ max 점수: 92점, min 점수: 76점
...
# 총 재고 금액 출력
price = sum(i[1] * i[2] for i in stocks)
price = sum((price * amount for _, price, amount in stocks))
price = sum([price * amount for _, price, amount in stocks])
print(f"총액: {price:,}원")  # ✅ 총액: 21,000원
...
total = sum(price * amount for _, price, amount in zip(*stocks))
print(f"총액: {total:,}원")
